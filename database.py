...
class Database:
    ...
    def init_database(self):
        ...
        # Tabela de controle de envios por horário (evita duplicatas)
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS sent_schedules (
                canal_id INTEGER NOT NULL,
                horario TEXT NOT NULL,
                date TEXT NOT NULL,
                PRIMARY KEY (canal_id, horario, date)
            )
        ''')
        
        # Índices para melhor performance
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_sent_schedules_date ON sent_schedules(date)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_media_cycle_date ON media_cycle(cycle_date)')
    ...
    def limpar_registros_antigos(self):
        ...
        hoje = datetime.now(BRASILIA_TZ).date().isoformat()
        ontem = (datetime.now(BRASILIA_TZ) - timedelta(days=1)).date().isoformat()
        
        conn = self.get_connection()
        cursor = conn.cursor()
        
        cursor.execute("DELETE FROM sent_schedules WHERE date < ?", (ontem,))
        sent_removidos = cursor.rowcount
        
        cursor.execute("DELETE FROM media_cycle WHERE cycle_date < ?", (ontem,))
        cycle_removidos = cursor.rowcount
        
        conn.commit()
        conn.close()
        
        if sent_removidos > 0 or cycle_removidos > 0:
            logger.info(f"🧹 Limpeza: {sent_removidos} envios e {cycle_removidos} ciclos antigos removidos")
    ...
    # Nota: As funções get_media_cycle e pop_media_cycle foram movidas para o scheduler
    # para melhor organização e performance. O scheduler agora gerencia o ciclo diretamente.
